...
from employees.models.tinydb.todo import Todo as Model
...
class Todo(PowHandler):
    ...
    for windows: (the quotes need to be escape in cmd.exe)
      ...
      POST:   curl -H "Content-Type: application/json" -X POST -d "{ \"title\" : \"first todo\" }" http://localhost:8080/todo
      GET:    curl -H "Content-Type: application/json" -X GET http://localhost:8080/todo
      PUT:    curl -H "Content-Type: application/json" -X PUT -d "{ \"id\" : \"1\", \"text\": \"lalala\" }" http://localhost:8080/todo
      DELETE: curl -H "Content-Type: application/json" -X DELETE -d "{ \"id\" : \"1\" }" http://localhost:8080/todo
    """
    model=Model()
    ...
    def show(self, id=None):
        m=Model()
        res=m.find_by_id(id)
        self.success(message="todo show", data=res)
        
    def list(self):
        m=Model()
        res = m.get_all()  
        self.success(message="todo, index", data=res)         
    
    def page(self, page=0):
        m=Model()
        res=m.page(page=int(page), page_size=myapp["page_size"])
        self.success(message="todo page: #" +str(page), data=res )  
    ...
    def edit(self, id=None):
        ...
        try:
            ...
            res = m.find_by_id(id)
            self.success(message="todo, edit id: " + str(id), data=res)
        except Exception as e:
            self.error(message="todo, edit id: " + str(id) + "msg: " + str(e) , data=None)

    @tornado.web.authenticated
    def new(self):
        m=Model()
        self.success(message="todo, new",data=m)
    ...
    def update(self, id=None):
        ...
        try:
            ...
            res.upsert()
            self.success(message="todo, successfully updated " + str(res.id), 
                data=res, format="json")
        except Exception as e:
            self.error(message="todo, error updating: " + str(m.id) + "msg: " + str(e), data=data_json, format="json")
    ...
    def search(self):
        m=Model()
        return self.error(message="todo search: not implemented yet ")